...
INSERT INTO Empleados (Nombre, Puesto, Departamento, SucursalID, Rol) VALUES 
('John Doe', 'Gerente', 'Administración', 1, 'Vendedor'), ('Jane Smith', 'Subgerente', 'Ventas', 1, 'Vendedor'), ('Bill Jones', 'Cajero', 'Ventas', 1, 'Vendedor'), ('Alice Johnson', 'Cocinero', 'Cocina', 1, 'Vendedor'), ('Tom Brown', 'Barista', 'Cafetería', 1, 'Vendedor'), ('Emma Davis', 'Repartidor', 'Logística', 1, 'Mensajero'), ('Lucas Miller', 'Atención al Cliente', 'Servicio', 1, 'Vendedor'), ('Olivia García', 'Encargado de Turno', 'Administración', 1, 'Vendedor'), ('Ethan Martinez', 'Mesero', 'Restaurante', 1, 'Vendedor'), ('Sophia Rodriguez', 'Auxiliar de Limpieza', 'Mantenimiento', 1, 'Vendedor');
...
INSERT INTO TiposDePago (Nombre) VALUES
...
--Esta linea genera error debido a que esa relcionado con otra tabla, decido omitirla
...
-----------------------------RESPONDO LAS PREGUNTAS EN COMENTARIOS PARA QUE NO SE EJECUTEN A LA HORA DE LA CORRECCIÓN---------------------------------------------

--Pregunta: ¿Cuál es la cantidad total de registros únicos en la tabla de órdenes?

--SELECT COUNT(OrdenID) AS TotalRegistros FROM Ordenes

--Pregunta: ¿Cuántos empleados existen en cada departamento?

--SELECT COUNT(EmpleadoID) AS TotalEmpleados, Departamento FROM Empleados GROUP BY Departamento

--Pregunta: ¿Cuántos productos hay por código de categoría?

--SELECT COUNT(ProductoID) AS TotalProductos, CategoriaID FROM Productos GROUP BY CategoriaID

--Pregunta: ¿Cuántos clientes se han importado a la tabla de clientes?

--SELECT COUNT(ClientesID) AS TotalClientes FROM Clientes
...
--SELECT SucursalID, AVG(TotalCompra) AS PromedioIngresos, AVG(KilometrosRecorrer) AS PromedioKilometros FROM Ordenes GROUP BY SucursalID HAVING AVG(TotalCompra) > 1000 ORDER BY PromedioKilometros ASC